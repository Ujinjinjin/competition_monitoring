...
class Observer:
        ...
        for student in observable_students:
            for saved_student in saved_data:
                if student.name == saved_student.name and student.position != saved_student.position:
                    await self.notify(student.name, saved_student.position, student.position)
    ...
    async def notify(self, student_name: str, old_position: int, new_position: int):
        """Notify observer of changes"""

        if new_position > 92 > old_position:
